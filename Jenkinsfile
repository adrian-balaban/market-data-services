...
pipeline {
    ...
    stages {
        ...
        stage("Build&Deploy") {
            steps {
              script {
                withEnv(['useBash=${user_parameters.useBash}']) {
                    withKubeConfig(caCertificate: '',
                     ...
                     sh """
                        if [ ${useBash} = true ]; then
                           ...
                           cd ./infra/k8s && ./deployAll.sh      -build ${user_parameters.build} -test ${user_parameters.test} -n ${k8s_namespace} -tag ${user_parameters.tag_root}-${env.BRANCH_NAME} -registry ${env.registry}
                        else
                           echo "Executing with argoCD script..."
                           cd  ./infra/argo && ./deployWithArgo.sh -build ${user_parameters.build} -test ${user_parameters.test} -n ${k8s_namespace} -tag ${user_parameters.tag_root}-${env.BRANCH_NAME} -registry ${env.registry} -branch ${env.BRANCH_NAME} -env test
                        fi
                    """
                    }
                }
