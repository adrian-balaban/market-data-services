            ...
            steps {
                script {
                    try {
                        ...
                        echo "CONFIG: env.useBash: " + env.useBash
                    }
                }
                script {
                  echo "CONFIG: env.useBash: " + env.useBash
              ...
              script {
                 withKubeConfig(caCertificate: '',
                   ...
                   restrictKubeConfigAccess: true, serverUrl: 'https://192.168.192.96:6443') {
                     sh "cd infra/k8s && ./deployWithBashOrArgo.sh -with_bash ${env.useBash} -build ${env.build} -test ${env.test} -n ${k8s_namespace} -tag ${env.tag} -registry ${env.registry} -env test -branch ${env.BRANCH_NAME}"
                 }
              }
            }
        }
