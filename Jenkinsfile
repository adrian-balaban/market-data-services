pipeline {
    ...
    parameters {
        choice(choices: ['useBash', 'useArgoCD'], description: 'Do you want to deploy using ArgoCD?', name: 'boolUseBash')
    }
    options {
    ...
    stages {
        ...
        stage('Build&Deploy with bash') {
            when { expression { ${params.boolUseBash} == 'true' } }
            steps {
              script {
                 echo "boolUseBash: ${params.boolUseBash}"
        ...
        stage('Build&Deploy with argoCD') {
            when { expression { ${params.boolUseBash} == 'false' } }
            steps {
              script {
                 echo "boolUseBash: ${params.boolUseBash}"
                 def k8s_namespace = env.BRANCH_NAME == 'master' ? 'test' : env.BRANCH_NAME
