...
ps aux | grep -i "kubectl port-forward"  | grep -i "6379" | grep -v grep | awk {'print $2'} | xargs --no-run-if-empty kill || true
ps aux | grep -i "kubectl port-forward"  | grep -i "38080" | grep -v grep | awk {'print $2'} | xargs --no-run-if-empty kill || true
