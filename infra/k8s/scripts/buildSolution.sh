...
pushd ../../services/docker/fx-market-connector && ./build-image.sh -tag ${TAG} -registry ${DOCKER_REGISTRY} && popd
pushd ../../services/docker/fx-market-processor && ./build-image.sh -tag ${TAG} -registry ${DOCKER_REGISTRY} && popd
pushd ../../services/docker/flink-orchestrator && ./build-image.sh -tag ${TAG} -registry ${DOCKER_REGISTRY} && popd
